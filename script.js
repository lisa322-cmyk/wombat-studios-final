...
const MY_API_KEY =
    "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";
...
const wombatScript = {
    approval: [
        {
            phrase: "О, а це вже цікаво. Продовжуйте в тому ж дусі, і, можливо, щось вийде.",
            meme: "Це база.",
        },
        {
            phrase: "Хм. Непогано. Навіть я здивований.",
            meme: "Пацани, вапщєта кайфую!",
        },
        { phrase: "Так, оце вже схоже на кіно. Чотко!", meme: "Чотко!" },
    ],
    disapproval: [
        {
            phrase: "Ні, ні і ще раз ні. Це не мистецтво, це халтура.",
            meme: "Це фіаско, братан.",
        },
        {
            phrase: "Режисере, ви взагалі читали сценарій?",
            meme: "Трошки текст по-дєбільному написаний.",
        },
        {
            phrase: "О Боже, мої очі... Що це таке?",
            meme: "Боже, яке кончене!",
        },
        {
            phrase: "Все, досить. У вас лице скучне, і постер такий самий.",
            meme: "Міша, всьо переписуєм, у тебе скучноє ліцо.",
        },
    ],
    confusion: [
        {
            phrase: "Що-що? Ви серйозно? Я такого ще не бачив.",
            meme: "Це шо, прикол?",
        },
        {
            phrase: "Цікавий хід... Дуже сміливий. Можливо, навіть занадто.",
            meme: "А так можна було?",
        },
        {
            phrase: "Я не впевнений, що зрозумів ваш геніальний задум.",
            meme: "Ти що, дурний?",
        },
    ],
    action: [
        {
            phrase: "Гаразд, не подобається. Робимо все з нуля. Не сперечайтесь.",
            meme: "Давай по новій, Міша, все хуйня!",
        },
        {
            phrase: "Так, мені потрібен чіткий план. Що робимо далі?",
            meme: "Маєте якусь стратегію?",
        },
        { phrase: "Добре, я вас почув. Починаємо.", meme: "Нам своє робить!" },
    ],
    neutral: [
        { phrase: "Ну, подивимось...", meme: "Та ти шо!" },
        { phrase: "Продовжуйте, я уважно слухаю.", meme: "Юля!" },
        {
            phrase: "Так, хвилинку, мені треба подумати... і кави.",
            meme: "Йдемо по каву.",
        },
        {
            phrase: "Цікаво, до чого це нас приведе.",
            meme: "Ну все, приїхали.",
        },
    ],
};
...
function getWombatReaction(userCommand, isError = false) {
    if (isError) {
        const errorMemes = [
            {
                phrase: "Це фіаско, братан. Знову щось пішло не так.",
                meme: "Це фіаско, братан.",
            },
        ];
        return errorMemes[0];
    ...
    for (const category in keywords) {
        for (const keyword of keywords[category]) {
            if (command.includes(keyword)) {
                const reactions = wombatScript[category];
    ...
    return neutralReactions[
        Math.floor(Math.random() * neutralReactions.length)
    ];
}
...
async function makeApiCall(messages) {
    if (
        MY_API_KEY.trim().includes("сюди-вставте-ваш") ||
        MY_API_KEY.trim() === ""
    ) {
    ...
    const response = await fetch(
        "https://openrouter.ai/api/v1/chat/completions",
        {
            ...
            headers: {
                ...
                Authorization: `Bearer ${MY_API_KEY.trim()}`,
            },
            body: JSON.stringify({
    ...
    if (!response.ok) {
        ...
        throw new Error(
            `Помилка API: ${errorData.error.message || response.statusText}`,
        );
    }
...
generatePosterBtn.addEventListener("click", async () => {
    if (!storyDescription.value) {
        alert("Опишіть ідею фільму!");
        return;
    }
    if (userImagesBase64.length === 0) {
        alert("Хто гратиме головні ролі? Завантажте хоча б одне фото!");
        return;
    }
    ...
    try {
        const finalGenre = customGenre.value.trim()
            ? customGenre.value.trim()
            : genreSelector.value;
        ...
        **Text Elements:** Add a relevant short title and a witty, one-sentence "critic's review" at the bottom.`;
        ...
        if (data.choices[0].finish_reason === "content_filter") {
            throw new Error(
                "Запит було заблоковано фільтром безпеки. Спробуйте інший, більш нейтральний опис або інші фото.",
            );
        }
        ...
        theatricalReleaseDiv.innerHTML =
            '<h3>Перший дубль: "Театральна Версія"</h3>';
...
directorCutBtn.addEventListener("click", () => {
    ...
    addMessageToChat(
        "Так, режисере, сідайте. У нас є матеріал, але він 'сирий'.\nЩо будемо міняти, щоб зробити з цього цукерочку?",
        "wombat",
    );
...
sendDirectorCommandBtn.addEventListener("click", async () => {
    ...
    const messages = [
        {
            ...
            content: [
                ...
                { type: "image_url", image_url: { url: currentPosterUrl } },
            ],
        },
    ];
    ...
    try {
        ...
        if (data.choices[0].finish_reason === "content_filter") {
            throw new Error(
                "Запит було заблоковано фільтром безпеки. Спробуйте іншу команду.",
            );
        }
        if (
            !data.choices[0].message.images ||
